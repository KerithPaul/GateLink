...
export function paymentMiddleware(
  ...
  onPaymentSettled?: PaymentCallback
) {
  const { verify, settle, supported } = useFacilitator(facilitator); // TODO: remove this
  const x402Version = 1;
  ...
  return async function paymentMiddleware(
    ...
    try {
      ...
      // Extract payer address from payment transaction
      if (onPaymentSettled && settleResponse.transaction) {
        try {
          const paymentTxn = decodedPayment.payload.paymentGroup[
          ...
          const linkId = (req as any).linkId; // Link ID passed from route handler

          // Call payment callback asynchronously (don't block response)
          onPaymentSettled({
            payerAddress,
